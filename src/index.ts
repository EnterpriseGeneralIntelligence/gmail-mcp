...
type NewMessage = {
  ...
  tracking_open_link?: string | undefined
  tracking_click_link?: string | undefined
  as_new_thread?: boolean | undefined
}
...
const constructRawMessage = async (gmail: gmail_v1.Gmail, params: NewMessage) => {
  ...
  if (thread && thread.messages?.length && !params.as_new_thread) {
    ...
    let toRecipients: string[] = [...(params.to || [])].map(email => email.trim()).filter(email => email.length > 0)
    ...
    if (toRecipients.length) message.push(`To: ${toRecipients.join(', ')}`)

    // Handle CC recipients - combine from thread reply-all and new params
    let ccRecipients: string[] = [...(params.cc || [])].map(email => email.trim()).filter(email => email.length > 0)
  ...
  if (params.bcc?.length) {
    ...
    if (bccEmailAddresses.length) message.push(`Bcc: ${bccEmailAddresses.join(', ')}`)
  }
...
<body>
  ...
  logToFile('constructRawMessage_complete', {
    ...
    htmlPart: fullMessage.substring(
      ...
      Math.min(fullMessage.indexOf('Content-Type: text/html') + 500, fullMessage.length)
    )
  })
